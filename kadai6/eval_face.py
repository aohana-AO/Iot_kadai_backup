...
def main():
	...
	s = F.softmax(y)
	hantei = '「入門許可」' if c == 1 else '「入門禁止」'
	print('判定結果は{}です。 {}'.format(hantei, s.data))
	print(c)
	#c==1のとき登録者、c==0のとき非登録者。

	try:
		f = open('判定結果.txt', 'x',encoding='utf-8')
	except FileExistsError:
		f = open('判定結果.txt', 'w',encoding='utf-8')
		print('新規作成できなかったため上書きしました。')


	f.write('判定結果は{}です。 {}'.format(hantei, s.data))
